...
MapViewport::MapViewport(const Topology &topology)
{
    ...
    scale = 1000.0 / std::max({projectedWidth, projectedHeight, 0.000001});
}
...
QPointF MapViewport::mapPosition(double latitude, double longitude) const
{
    ...
    return {(projectedPosition.getX() - minimumProjectedX) * scale, (maximumProjectedY - projectedPosition.getY()) * scale};
}
