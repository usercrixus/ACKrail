...
double Geography::distanceKilometers(double latitude1, double longitude1, double latitude2, double longitude2)
{
    ...
    const double centralAngle = 2.0 * std::atan2(std::sqrt(boundedHaversine), std::sqrt(1.0 - boundedHaversine));
...
Point Geography::webMercator(double latitude, double longitude)
{
    ...
    return {longitude * DegreesToRadians, std::log(std::tan(std::numbers::pi / 4.0 + latitudeRadians / 2.0))};
}
